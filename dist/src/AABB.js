/**
 * @file AABB
 * @see uon.math.AABB
 * @author Gabriel Roy <gab@uon.io>
 * @ignore
 */
import { Vector3 } from "./Vector3";
/**
 * 3D axis aligned bounding box
 * @memberOf uon.math
 */
var AABB = (function () {
    /**
     * An axis-aligned bounding-box
     *
     *
     */
    function AABB(min, max) {
        this.min = (min !== undefined) ?
            min : new Vector3(Infinity, Infinity, Infinity);
        this.max = (max !== undefined) ?
            max : new Vector3(-Infinity, -Infinity, -Infinity);
    }
    /**
     * Asign new values to this box
     */
    AABB.prototype.set = function (min, max) {
        this.min.copy(min);
        this.max.copy(max);
        return this;
    };
    /**
     * Compute a box around the given points
     */
    AABB.prototype.setFromPoints = function (points) {
        this.empty();
        for (var i = 0, il = points.length; i < il; i++) {
            this.expand(points[i]);
        }
        return this;
    };
    /**
     * Empty this box
     */
    AABB.prototype.empty = function () {
        this.min.x = this.min.y = this.min.z = Infinity;
        this.max.x = this.max.y = this.max.z = -Infinity;
        return this;
    };
    /**
     * Test for equality with another box
     */
    AABB.prototype.equals = function (box) {
        return box.min.equals(this.min) && box.max.equals(this.max);
    };
    /**
     * Computes the center of the box and return its value
     *
     * @returns {uon.math.Vector3}
     */
    AABB.prototype.getCenter = function (output) {
        var result = output || new Vector3();
        return result.copy(this.min).add(this.max).multiplyScalar(0.5);
    };
    /**
     * Computes the size of the box for each axis
     *
     * @returns {uon.math.Vector3}
     */
    AABB.prototype.getSize = function (output) {
        var result = output || new Vector3();
        return result.copy(this.max).subtract(this.min);
    };
    /**
     * Test for containment
     *
     * @param {uon.math.Vector3}
     *            point
     */
    AABB.prototype.containsPoint = function (point) {
        if (point.x < this.min.x || point.x > this.max.x
            || point.y < this.min.y || point.y > this.max.y
            || point.z < this.min.z || point.z > this.max.z) {
            return false;
        }
        return true;
    };
    /**
     * Test if a box is contained within this one
     *
     * @param {uon.math.AABB}
     *            box
     * @returns {Boolean}
     */
    AABB.prototype.containsBox = function (box) {
        if ((this.min.x <= box.min.x) && (box.max.x <= this.max.x)
            && (this.min.y <= box.min.y) && (box.max.y <= this.max.y)
            && (this.min.z <= box.min.z) && (box.max.z <= this.max.z)) {
            return true;
        }
        return false;
    };
    /**
     * Test for intersection with another box
     *
     * @param {uon.math.AABB}
     *            box
     */
    AABB.prototype.intersects = function (box) {
        if (box.max.x < this.min.x || box.min.x > this.max.x
            || box.max.y < this.min.y || box.min.y > this.max.y
            || box.max.z < this.min.z || box.min.z > this.max.z) {
            return false;
        }
        return true;
    };
    /**
     * Expand the box to contain a point
     */
    AABB.prototype.expand = function (point) {
        this.min.min(point);
        this.max.max(point);
        return this;
    };
    /**
     * Scale the box
     */
    AABB.prototype.scale = function (scalar) {
        this.min.multiplyScalar(scalar);
        this.max.multiplyScalar(scalar);
        return this;
    };
    /**
     * Expand this box to contain another box
     */
    AABB.prototype.merge = function (box) {
        this.min.min(box.min);
        this.max.max(box.max);
        return this;
    };
    /**
     * Creates a copy of this box
     */
    AABB.prototype.clone = function () {
        return new AABB().copy(this);
    };
    /**
     * Copy values form another box
     */
    AABB.prototype.copy = function (box) {
        this.min.copy(box.min);
        this.max.max(box.max);
        return this;
    };
    return AABB;
}());
export { AABB };
;
//# sourceMappingURL=AABB.js.map